<template>
    <el-header>
        <div class="left-panel">
            <el-button type="primary" icon="el-icon-plus" @click="add" text>{{ $t("knowledge.btnAdd") }}</el-button>
        </div>
        <div class="right-panel">
            <div class="right-panel-search">
                <el-input v-model="search.keyword" :placeholder="$t('knowledge.namePlaceholder')" clearable></el-input>
                ...
                <el-card class="card card-item">
                    <div class="pointer-cursor" @click="detail(item)">
                        ...
                        <ul>
                            ...
                            <li>
                                <p>{{ $t('knowledge.lastModificationTime') }}:{{item.lastModificationTime ||item.creationTime}}</p>
                            </li>
                        </ul>
                    </div>
                    <div class="bottom">
...
export default {
    ...
    methods: {
        async getList() {
            this.isLoading = true;
            try {
                var res = await this.$API.aigc.knowledge.list.post(this.search);
                this.list = res.items;
            } finally {
        ...
        detail(o) {
            this.$router.push({
                path: '/aigc/resource/knowledge/detail',
                query: {
                    id: o.id,
                },
            });
        ...
        async del(o) {
            this.$confirm(
                ...
                .then(async () => {
                    await this.$API.aigc.knowledge.delete.post(o.id);
                    await this.getList();
                })
                .catch(() => {});
        },
