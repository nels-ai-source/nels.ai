...
public sealed class FunctionToolCall
{
    ...
    internal FunctionToolCall(IFunctionCall functionToolCall, string? nameSeparator = "_")
    {
        ...
        int separatorPos = fullyQualifiedFunctionName.IndexOf(_nameSeparator, StringComparison.Ordinal);
        if (separatorPos >= 0)
        {
            pluginName = fullyQualifiedFunctionName.AsSpan(0, separatorPos).Trim().ToString();
            functionName = fullyQualifiedFunctionName.AsSpan(separatorPos + _nameSeparator.Length).Trim().ToString();
        }
    ...
    public string FullyQualifiedName
        => this._fullyQualifiedFunctionName
            ??= string.IsNullOrEmpty(this.PluginName) ? this.FunctionName : $"{this.PluginName}{_nameSeparator}{this.FunctionName}";
