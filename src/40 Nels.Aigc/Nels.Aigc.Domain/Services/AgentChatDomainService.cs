...
{
    public virtual async Task InsertAgentChatAsync(AgentChat agentChat, CancellationToken cancellation = default)
    {
        ...
        if (agentChat.StepLogs.Count != 0)
        {
            ...
            if (stepLogs != null)
            {
                await stepLogRepository.InsertManyAsync(stepLogs, cancellationToken: cancellation);
            }
        }
    }
}
